...
\( \ctx \) and execution level \( \pc \), full execution of command \( c \) will modify the context to finally be $\ctx'$. Importantly, it is allowed to strengthen the final context \( \ctx' \) even when unnecessary, as illustrated in the rules for Skip, Assign, Input and Output. This is important, as exitting an If, we must merge the final contexts from the two branches.
