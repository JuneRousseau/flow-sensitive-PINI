...
Section simpl_gmap.
  ...
  Fixpoint remove_key {A: Type} k (rm: @rgmap A) :=
    match rm with
    | Ins k' a rm => if bool_decide (k = k') then remove_key k rm else Ins k' a (remove_key k rm)
    | Del k' rm => if bool_decide (k = k') then remove_key k rm else Del k' (remove_key k rm)
    | Symb => Symb
    end.
  ...
  Proof.
    ...
    - destruct (bool_decide (k = k0)); simpl; lia.
    - destruct (bool_decide (k = k0)); simpl; lia.
  Qed.
  ...
  Proof.
    induction rm; simpl; auto.
    - intros.
      destruct (bool_decide (k0 = k)) eqn:Heq; [apply bool_decide_eq_true in Heq | apply bool_decide_eq_false in Heq].
      + subst k0. rewrite H.
